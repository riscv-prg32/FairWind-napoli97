...
python3 -m prg32 cartridge build "$HERE/src/game.c" --portable --multiplayer --entry-prefix nacup --name NaCup-napoli97 --audio-block "$BUILD/nacup-audio.block" --out "$BUILD/nacup-base.prg32"
...
for f in "$STORE"/*.prg32; do test "$(wc -c < "$f")" -le 131072 || { echo "$f exceeds 128 KiB" >&2; exit 3; }; done
(cd "$DIST" && sha256sum store/*.prg32 NaCup-napoli97-3.0.0-store.zip > SHA256SUMS)
echo "Built portable 128 KiB profile: $DIST/NaCup-napoli97-3.0.0-store.zip"
