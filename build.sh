...
if [[ -n "${CARTRIDGE_STORE_ROOT:-}" ]]; then python3 "$HERE/tools/check_store_bundle.py" "$DIST/NaCup-napoli97-3.0.0-store.zip" "$CARTRIDGE_STORE_ROOT"; else echo "CARTRIDGE_STORE_ROOT not set: skipping Store intake check" >&2; fi
