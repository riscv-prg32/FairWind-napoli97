...
VERSION="$(python3 -c 'import json,sys;print(json.load(open(sys.argv[1]))["version"])' "$HERE/metadata/metadata.json")"; BUNDLE="FairWind-napoli97-$VERSION-store.zip"
BUILD="$HERE/build"; DIST="$HERE/dist"; STORE="$DIST/store"; rm -rf "$BUILD" "$STORE"; mkdir -p "$BUILD" "$STORE"
cd "$ROOT"
python3 tools/prg32audio_pack.py "$HERE/audio.json" --out "$BUILD/fairwind-audio.block"
python3 "$HERE/tools/prg32_cli_64.py" cartridge build "$HERE/src/game.c" --portable --multiplayer --entry-prefix fairwind --name FairWind-napoli97 --audio-block "$BUILD/fairwind-audio.block" --out "$BUILD/fairwind-base.prg32"
for arch in esp32c6 qemu; do python3 -m prg32 store attach-metadata "$BUILD/fairwind-base.prg32" --metadata "$HERE/metadata/metadata.json" --icon "$HERE/assets/generated/icon.png" --screenshot "$HERE/assets/generated/screenshot.png" --colophon "$HERE/metadata/colophon.json" --architecture "$arch" --out "$STORE/FairWind-napoli97-$arch.prg32"; done
