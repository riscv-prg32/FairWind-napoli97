...
static fairwind_sprite_t cup_sprite;static boat_t boats[4],prop;static screen_t screen;
static uint32_t last_input,frame,rng=0x97ca1997u,wind_seed;static int team_sel,menu,points[4],race_no,money,hull,sails,crew,strategy,sponsor,wins,last_prize,tws10,race_clock,start_clock,race_frames,signal_sound_frames,course_sel;static int32_t wind_t;static uint16_t twd,cam_h;static uint8_t multiplayer,peer_count,player_rank,local_ready,start_line_active,top_view_mode,ab_combo,result_order[4],net_live[4];static uint32_t net_ids[4];
...
static void setup_sprites(void){sprite(&cup_sprite,fairwind_cup_planes,fairwind_palette,32,40,1,4,0);}
...
static void play_start_signal(int long_signal){prg32_audio_play_track((uint16_t)(long_signal?4:3));signal_sound_frames=long_signal?42:24;}
...
static int frames_per_sim_second(void){return 60/time_scale();}
...
static void update_rig(boat_t *b,int local){
    int scale=time_scale(),awa,side,lim,step,target,cr=crew_of(b),heel;int32_t tws=tws_mms(),wf,ws;int8_t nt;
    ...
    step=((b->boom<0)!=(side<0)&&absi(b->boom)>8?12:3)*scale/4;if(step<1)step=1;
    ...
    if(local){
        int32_t up=65535L*scale/(60*(32-2*cr)),down=65535L*scale/(60*(20-cr));
...
static void sail(boat_t *b){
    int scale=time_scale(),twa=absi(BAM2DEG(b->twa)),cr=crew_of(b),perf,tau;int32_t v=b->v>>8,target,old_y=b->y,q,step,drift,vv;
    ...
    b->v+=((target<<8)-b->v)*scale/(60*tau);
    b->v-=(b->v/100)*absi(b->rudder)*scale/3000;if(b->v<0)b->v=0;
    v=b->v>>8;vv=v<4100?v:4100;
    q=(int32_t)b->rudder*(300+900*vv/4100)*182/100;
    q=q*scale*(92+2*cr)/100*256/6000+b->hacc;
    ...
    step=v*scale/60;b->x+=(step*fsin(b->heading))>>14;b->y+=(step*fcos(b->heading))>>14;
    drift=tws_mms()*15/1000*scale/60;b->x-=(drift*fsin(twd))>>14;b->y-=(drift*fcos(twd))>>14;
...
static void helm_ai(boat_t *b){
    ...
    slew_rudder(b,clamp(BAM2DEG((int16_t)(want-b->heading))*5+(int)(rnd()%3)-1,-100,100),10);
...
static void start_race(void){
    ...
    race_clock=0;start_clock=START_COUNTDOWN_SECONDS;race_frames=0;start_line_active=1;top_view_mode=0;signal_sound_frames=0;ab_combo=1;
    ...
    cam_h=boats[0].heading;screen=ST_RACE;play_start_signal(0);
}
...
static void update_race(uint32_t in,uint32_t released){
    ...
    race_frames++;if(race_frames>=frames_per_sim_second()){race_frames=0;wind_t++;update_wind();if(start_clock>0){start_clock--;if(start_clock==300||start_clock==240||start_clock==60)play_start_signal(0);else if(start_clock==0)play_start_signal(1);}else race_clock++;}
    ...
    cam_h=(uint16_t)(cam_h+(int16_t)(me->heading-cam_h)/6);
...
static void draw_team(void){int i;header("CHOOSE YOUR SYNDICATE");for(i=0;i<4;i++){int y=28+i*36;box(24,y,272,29,i==team_sel?GOLD:GREY,NAVY);prg32_gfx_rect(34,y+7,20,15,teams[i].color);prg32_gfx_text8(64,y+6,teams[i].name,WHITE,NAVY);prg32_gfx_text8(64,y+17,teams[i].code,GOLD,NAVY);}prg32_gfx_text8(55,178,"LEFT/RIGHT  A CONFIRM",WHITE,SEA);}
static void statrow(int y,const char *n,int v,int s){int i;prg32_gfx_text8(30,y,n,s?GOLD:WHITE,NAVY);for(i=0;i<8;i++)prg32_gfx_rect(132+i*13,y,10,7,i<v?CYAN:GREY);}
...
static void rect_clip(int x,int y,int w,int h,uint16_t c){if(x<0){w+=x;x=0;}if(x+w>W)w=W-x;if(y<CLIP_TOP){h-=CLIP_TOP-y;y=CLIP_TOP;}if(y+h>CLIP_BOT)h=CLIP_BOT-y;if(w>0&&h>0)prg32_gfx_rect(x,y,w,h,c);}
/* Even-odd scanline fill, so concave sail outlines render correctly. */
static void fill_poly(const int *xs,const int *ys,int n,uint16_t c){
    int y,y0=9999,y1=-9999,i,j,k,m;
    for(i=0;i<n;i++){if(ys[i]<y0)y0=ys[i];if(ys[i]>y1)y1=ys[i];}
    if(y0<CLIP_TOP)y0=CLIP_TOP;if(y1>CLIP_BOT)y1=CLIP_BOT;
    for(y=y0;y<y1;y++){
        int cx[16];m=0;
        for(i=0;i<n;i++){j=i+1==n?0:i+1;if((y>=ys[i]&&y<ys[j])||(y>=ys[j]&&y<ys[i]))cx[m++]=xs[i]+(y-ys[i])*(xs[j]-xs[i])/(ys[j]-ys[i]);}
        for(i=1;i<m;i++){int v=cx[i];for(k=i;k>0&&cx[k-1]>v;k--)cx[k]=cx[k-1];cx[k]=v;}
        for(k=0;k+1<m;k+=2)rect_clip(cx[k],y,cx[k+1]-cx[k]+1,1,c);
    }
}
...
static void line2(int x0,int y0,int x1,int y1,uint16_t c){
    int dx=x1-x0,dy=y1-y0,n=absi(dx)>absi(dy)?absi(dx):absi(dy),i;int32_t x=(int32_t)x0*65536+32768,y=(int32_t)y0*65536+32768,sx,sy;
    if(n>1200)return;if(!n)n=1;sx=(int32_t)dx*65536/n;sy=(int32_t)dy*65536/n;
    for(i=0;i<=n;i++){int px=(int)(x>>16),py=(int)(y>>16);if(px>=0&&px<W&&py>=CLIP_TOP&&py<CLIP_BOT)prg32_gfx_pixel(px,py,c);x+=sx;y+=sy;}
}
...
static void panorama(int top,int off){
    ...
    while(i<end){int count=fairwind_background_rle8[i++],a=x+off,b=x+off+count;uint16_t color=fairwind_background_palette[fairwind_background_rle8[i++]];if(a<0)a=0;if(b>W)b=W;if(b>a)prg32_gfx_rect(a,y,b-a,1,color);x+=count;if(x>=320){x=0;y++;}}
}
...
static void draw_horizon(void){
    ...
    prg32_gfx_rect(0,18,W,20,SKY1);prg32_gfx_rect(0,38,W,20,SKY2);prg32_gfx_rect(0,58,W,HOR-58,SKY3);
    for(sx=0;sx<W;sx+=4){uint16_t wb=(uint16_t)(cam_h+(sx+2-160)*10430/FOCAL);int d=absi((int16_t)(wb-32768u)),hh;if(d<DEG(61)||d>DEG(128))continue;hh=6+(int)((fsin((uint16_t)(wb*3u))+fsin((uint16_t)(wb*7u+9000u))/2)>>12);prg32_gfx_rect(sx,HOR-hh,4,hh,HILL);}
    if(off>-W&&off<W)panorama(HOR-24,off);
    for(sx=HOR;sx<CLIP_BOT;sx++){int f=(sx-HOR)*48/(CLIP_BOT-HOR),d=(sx*5)&15;prg32_gfx_rect(0,sx,W,1,f<4?SEA1:f<16?(d<f-4?SEA3:SEA2):(d<(f-16)/2?SEA4:SEA3));}
}
...
static void draw_ripples(void){
    int i,j,sx,sy,w;int32_t cx,cy,drift=((int32_t)wind_t*6+(int32_t)(frame/4))%50000,dx=-(drift*fsin(twd))/16384,dy=-(drift*fcos(twd))/16384,gx,gy;
    ...
    for(i=-7;i<=7;i++)for(j=-7;j<=7;j++){
        ...
        proj(&p,&sx,&sy);w=top_view_mode?6:2+(int)(1200/p.z);rect_clip(sx-w/2,sy,w,1,(h&3)?RIPPLE:FOAM);
    }
}
...
static void draw_buoy(int x,int y,int next){
    ...
    if(ty>sy-2)ty=sy-2;rect_clip(sx-w,ty,2*w,sy-ty+1,BUOY);rect_clip(sx-w,(ty+sy)/2,2*w,1,WHITE);
    if(next&&(frame&16)){rect_clip(sx-w-3,ty-3,2*w+6,1,GOLD);rect_clip(sx-w-3,sy+2,2*w+6,1,GOLD);rect_clip(sx-w-3,ty-3,1,sy-ty+6,GOLD);rect_clip(sx+w+2,ty-3,1,sy-ty+6,GOLD);}
    if(next&&ty-12>CLIP_TOP&&sx>16&&sx<W-16)prg32_gfx_text8(sx-15,ty-12,"NEXT",GOLD,top_view_mode?SEA2:SEA1);
    if(next&&top_view_mode){int i;for(i=0;i<32;i++){v3 z=cam_pt(x*10+((MARK_ZONE*10*fsin((uint16_t)(i*2048)))>>14),y*10+((MARK_ZONE*10*fcos((uint16_t)(i*2048)))>>14),0);int zx,zy;if(plane_d(&z,0)<0||plane_d(&z,1)<0||plane_d(&z,2)<0||plane_d(&z,3)<0)continue;proj(&z,&zx,&zy);rect_clip(zx,zy,1,1,GOLD);}}
}
...
static void draw_wind_gauge(void){
    ...
    prg32_gfx_rect(2,20,58,62,NAVY);
    for(i=0;i<16;i++)prg32_gfx_rect(31+(int)((22*fsin((uint16_t)(i*4096)))>>14),46-(int)((22*fcos((uint16_t)(i*4096)))>>14),1,1,GREY);
    prg32_gfx_rect(30,40,3,13,WHITE);prg32_gfx_rect(31,38,1,2,WHITE);
    line2(31+(int)((22*fsin(rel))>>14),46-(int)((22*fcos(rel))>>14),31+(int)((8*fsin(rel))>>14),46-(int)((8*fcos(rel))>>14),GOLD);
    prg32_gfx_rect(30+(int)((8*fsin(rel))>>14),45-(int)((8*fcos(rel))>>14),3,3,GOLD);
    prg32_gfx_rect(30+(int)((18*fsin((uint16_t)b->awa))>>14),45-(int)((18*fcos((uint16_t)b->awa))>>14),3,3,CYAN);
    small_num(4,72,tws10,1,WHITE,NAVY);
    if(upwind&&absi(shift)>=3){int lift=shift*b->tack>0;prg32_gfx_text8(38,72,lift?"L":"H",lift?GREEN:RED,NAVY);digit(46,72,absi(shift),lift?GREEN:RED,NAVY);}
}
...
static void draw_minimap(void){
    ...
    prg32_gfx_rect(254,20,66,66,NAVY);prg32_gfx_rect(257,24,60,57,SEA2);prg32_gfx_rect(254,20,3,66,C6(4,3,2));prg32_gfx_rect(317,20,3,66,C6(1,3,1));prg32_gfx_rect(257,81,60,5,C6(4,4,3));
    for(i=258;i<316;i+=4)prg32_gfx_rect(i,23,2,1,GOLD);
    if(start_line_active)prg32_gfx_rect(map_x(-LINE_HALF),map_y(0),map_x(LINE_HALF)-map_x(-LINE_HALF),1,WHITE);else prg32_gfx_rect(map_x(-FINISH_HALF),map_y(FINISH_Y),map_x(FINISH_HALF)-map_x(-FINISH_HALF),1,nl>=len?GOLD:WHITE);
    for(i=0;i<len;i++)prg32_gfx_rect(map_x(course_x[course_sel][i])-1,map_y(course_y[course_sel][i])-1,2,2,BUOY);
    if(nl<len){
        nx=map_x(course_x[course_sel][nl]);ny=map_y(course_y[course_sel][nl]);
        if(frame&16)prg32_gfx_rect(nx-2,ny-2,4,4,GOLD);
    ...
    for(i=3;i>=0;i--){int bx=map_x(boats[i].x/1000),by=map_y(boats[i].y/1000);if(i){prg32_gfx_rect(bx-1,by-1,2,2,teams[boats[i].team&3].kite);continue;}line2(bx,by,bx+(int)((5*fsin(boats[0].heading))>>14),by-(int)((5*fcos(boats[0].heading))>>14),WHITE);prg32_gfx_rect(bx-1,by-1,3,3,WHITE);}
    line2(262+(int)((5*fsin(twd))>>14),76-(int)((5*fcos(twd))>>14),262,76,GOLD);prg32_gfx_rect(261,75,3,3,GOLD);
}
static void draw_start_signal(void){const char *msg;if(start_clock>300)msg="10 MIN - WARNING";else if(start_clock>240)msg="5 MIN - CLASS SIGNAL";else if(start_clock>120)msg="4 MIN - P FLAG UP";else if(start_clock>60)msg="2 MIN - ENTER THE BOX";else if(start_clock>0)msg="1 MIN - P FLAG DOWN";else msg="START - CLASS FLAG DOWN";prg32_gfx_rect(64,20,188,12,NAVY);prg32_gfx_text8(66,22,msg,start_clock?WHITE:GOLD,NAVY);if(time_scale()>RACE_TIME_SCALE)prg32_gfx_text8(64,34,">> x20",GOLD,SKY1);}
static void draw_hud(void){
    ...
    prg32_gfx_rect(0,CLIP_BOT,W,H-CLIP_BOT,NAVY);
    prg32_gfx_text8(4,182,start_clock?"T-":"R",start_clock?GOLD:WHITE,NAVY);clock_mmss(20,182,start_clock?start_clock:race_clock,start_clock?GOLD:WHITE,NAVY);
    small_num(68,182,(int)((b->v>>8)*100/5144),1,WHITE,NAVY);prg32_gfx_text8(100,182,"KT",GREY,NAVY);
    prg32_gfx_text8(122,182,"TWA",GREY,NAVY);small_num(148,182,absi(BAM2DEG(b->twa)),0,WHITE,NAVY);
    prg32_gfx_text8(180,182,b->kite==KITE_SPIN?"SPI":b->kite==KITE_GENN?"GEN":"JIB",b->kite?teams[b->team&3].kite:WHITE,NAVY);
    if(b->kite!=b->kite_want||(b->kite&&kp<100)){small_num(206,182,kp,0,GOLD,NAVY);prg32_gfx_text8(230,182,b->kite!=b->kite_want?"v":"^",GOLD,NAVY);}
    if(b->started&&b->leg<len){prg32_gfx_text8(250,182,"NEXT",GOLD,NAVY);digit(290,182,b->leg+1,GOLD,NAVY);}else if(b->started)prg32_gfx_text8(250,182,"FINISH",GOLD,NAVY);
    prg32_gfx_text8(4,191,"TRIM",GREY,NAVY);prg32_gfx_rect(40,192,92,6,C6(1,1,2));prg32_gfx_rect(40+opt-4,192,9,6,C6(0,3,1));prg32_gfx_rect(40+b->sheet,197,1,2,GOLD);prg32_gfx_rect(40+clamp(absi(b->boom),0,90),191,2,8,WHITE);
    prg32_gfx_text8(136,191,err>8?"LUFF":err<-12?"STALL":"GOOD",err>8?RED:err<-12?GOLD:GREEN,NAVY);
    ...
    else if(multiplayer){prg32_gfx_text8(184,191,"NET",GREEN,NAVY);digit(210,191,peer_count+1,GREEN,NAVY);prg32_gfx_text8(218,191,"/4",WHITE,NAVY);}
    if(msg)prg32_gfx_text8(184,191,msg,mc,NAVY);
}
static void draw_race(void){
    camera_setup();
    if(top_view_mode)prg32_gfx_rect(0,CLIP_TOP,W,CLIP_BOT-CLIP_TOP,SEA2);else draw_horizon();
    draw_land();draw_ripples();draw_lines();draw_objects();
    header(race_names[race_no]);draw_wind_gauge();draw_minimap();
...
void fairwind_draw(void){prg32_gfx_clear(SEA);if(screen==ST_TITLE)draw_title();else if(screen==ST_MODE)draw_mode();else if(screen==ST_TEAM)draw_team();else if(screen==ST_MANAGER)draw_manager();else if(screen==ST_BRIEF)draw_brief();else if(screen==ST_LOBBY)draw_lobby();else if(screen==ST_RACE)draw_race();else if(screen==ST_RESULT)draw_result();else draw_season();prg32_gfx_present();}
